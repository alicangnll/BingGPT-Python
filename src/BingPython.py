        ...
        if loaddata == 0:
            ...
            if js1["result"]["value"] == "UnauthorizedRequest":
                ...
                try:
                    ...
                    asyncio.create_task(BingPython.ping(wsscon))
                ...
                while not final:
                    try:
                        ...
                        try:
                            ...
                            return "An error occured! Please try again! \nCode (OK) -> Server (OK) -> Return (ERROR)"
                    for obj in objects:
                                ...
                                if response["arguments"][0]["messages"][0]["text"]:
                                    if "Searching the web for" in str(response["arguments"][0]["messages"][0]["text"]):
                                        ...
                                        if "Generating answers for you..." in str(response["arguments"][0]["messages"][0]["text"]):
                                            ...
                                            if response["arguments"][0]["throttling"][0]["maxNumUserMessagesInConversation"] is not None:
                                                 return "An error occured about get messsages (maxNumUserMessagesInConversation: " + response["arguments"][0]["throttling"][0]["maxNumUserMessagesInConversation"] + ")\nCode (OK) -> Server (OK) -> Return (ERROR)" 
                                            else:
                                                veri = str(response["arguments"][0]["messages"][lenarray-2]["text"])
                            elif response["type"] == 2:
                                if response["arguments"][0]["messages"][1]["text"]:
                                    if "Searching the web for" in str(response["arguments"][0]["messages"][1]["text"]):
                                        continue
                                    else:
                                        if "Generating answers for you..." in str(response["arguments"][0]["messages"][1]["text"]):
                                            continue
                                        else:
                                            lenarray = len(response["arguments"][0]["messages"])
                                            if response["arguments"][0]["throttling"][0]["maxNumUserMessagesInConversation"] is not None:
                                    ...
                                    return "An error occured about get messsages (Type more than 3. Connection lost)\nCode (OK) -> Server (OK) -> Return (ERROR)" 
                        except(KeyError):
                            continue
                        except:
                            return "An error occured about get messsages (Response error)\nCode (OK) -> Server (OK) -> Return (ERROR)" 
